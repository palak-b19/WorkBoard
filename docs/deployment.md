...
## Day 23 – Full-Flow E2E, Edge-Cases & Performance

Environment

- Backend: Heroku (`task-mvp-backend`) – main @ 2025-07-08
- Frontend: local Vite dev server (`main`) – Cypress v14.5, Node v22
- Test user: autogenerated `fullflow<ts>@example.com`

### Automated E2E (Cypress)
Spec: `cypress/e2e/full-flow.cy.js`

| Flow                           | Result |
|--------------------------------|--------|
| Register → Dashboard redirect  | ✅ |
| Create 2 boards                | ✅ |
| Add 5 tasks, delete 1          | ✅ |
| Search "urgent" filter         | ✅ – highlights only matches |
| Board deletion                 | ✅ – list & analytics refresh |
| Analytics cards updated        | ✅ – totals 4 / 1 / 1 |
| Responsive layouts (1024/1280) | ✅ |

All assertions passed locally – see CI artefact # E2E-23.

### Edge-case Perf Sweep
Spec: `cypress/e2e/performance/performance.cy.js` (10 boards × 50 tasks)

```
--- Performance Summary (ms) ---
register     count=1   avg=2221  min=2221  max=2221
createBoard  count=10  avg= 285  min= 280  max= 299
createTask   count=500 avg= 430  min= 396  max=1228
getAnalytics count=1   avg= 276  min= 276  max= 276
getBoards    count=1   avg= 273  min= 273  max= 273
searchTasks  count=1   avg= 275  min= 275  max= 275
```

Observations

- All API endpoints stay **< 2 s** (avg & max) except the *first* user registration (cold-start 2.2 s) – acceptable.
- Task creation under heavy load averages **430 ms**; slowest 1.2 s – within SLA.
- Read endpoints consistently < 300 ms.

### Manual QA
Executed full flow end-to-end with Chrome @1280 × 800 – matches automated results. Accessibility labels verified via NVDA quick-nav; no violations found.

### Next-Steps (Day 24)
- Backend: expand edge-case suite (invalid JWT, malformed IDs) into supertest.
- Frontend: polish low-vision contrast on drag handles.
- Integrate perf thresholds into spec (fail if >2 s).
